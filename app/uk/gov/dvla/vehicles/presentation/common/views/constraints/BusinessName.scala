...
object BusinessName {
  ...
  def validBusinessName: Constraint[String] = {
      val name = "constraint.validBusinessName"
      Constraint[String](name) {
        case str: String =>
          pattern(Pattern.r, name, "error.validBusinessName").apply(str) match {
            case invalid: Invalid => invalid
            case Valid if balanceCount(str) != 0 => Invalid(ValidationError("error.invalidBraces"))
            case Valid if hasOddNumberOfQuotes(str) => Invalid(ValidationError("error.oddNumberOfQuotes"))
            case _ => Valid
          }
      }
    }

...
object NonFutureDate {
  ...
  val formatter = new Formatter[LocalDate] {
    def bind(key: String, data: Map[String, String]): Either[Seq[FormError], LocalDate] = {
      ...
      dateOfBirth.toRight(Seq[FormError](FormError(key, "error.nonFutureDate.invalid")))
    }
  ...
  def constraint(name: String) = Constraint[LocalDate](name) {
    case d: LocalDate =>
      if (d.isAfter(LocalDate.now)) Invalid(ValidationError(Messages("error.nonFutureDate.inTheFuture")))
      else Valid
  }
}

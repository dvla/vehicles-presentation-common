 ...
 */
trait CommonGlobalSettings extends GlobalSettings {

  def serviceName: String
  def injector: Injector
  def viewNotFound(request: RequestHeader): Appendable
  def errorStrategy: ErrorStrategyBase
  ...
  override def onStart(app: Application) {
    Logger.info(s"$serviceName Started") // used for operations, do not remove
  ...
  override def onStop(app: Application) {
    super.onStop(app)
    Logger.info(s"$serviceName Stopped") // used for operations, do not remove
  }

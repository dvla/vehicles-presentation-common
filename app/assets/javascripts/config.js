require.config({
    paths: {
        'jquery' : '../lib/jquery/jquery',
        'global-helpers': '../lib/vehicles-presentation-common/javascripts/global-helpers',
        'header-footer-only': '../lib/vehicles-presentation-common/javascripts/header-footer-only',
        'address-picker': '../lib/vehicles-presentation-common/javascripts/address-picker',
        'page-init': '../lib/vehicles-presentation-common/javascripts/page-init',
        'autofill-todays-date': '../lib/vehicles-presentation-common/javascripts/autofill-todays-date',
        'config': '../lib/vehicles-presentation-common/javascripts/config'
    }
});

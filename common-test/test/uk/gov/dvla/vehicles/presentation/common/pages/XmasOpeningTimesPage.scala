...
class XmasOpeningTimesPage(implicit driver: SearchContext) extends Page {

  final val address = "/xmas-opening-times"
  override val url: String = WebDriverFactory.testUrl + address.substring(1)
  final override val title: String = "View Christmas opening times"

  lazy val xmasOpeningTimesLink = driver.findElements(By.id("xmas-opening-times"))
  lazy val link: WebElement = xmasOpeningTimesLink.get(0)
}

object XmasOpeningTimesPage {
  def instance(implicit driver: SearchContext) = new XmasOpeningTimesPage
}